...
public class PersonRestAdapter {
         ...
         */
        @Operation(summary = "Obtener todas las personas", description = "Recupera la lista completa de personas registradas en el sistema.")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.read', 'client.write')") // Solo los usuarios con autoridad
                                                                                      // 'admin.full' pueden acceder a
                                                                                      // esta información
        @GetMapping("/v1/api")
         ...
         */
        @Operation(summary = "Obtener persona por ID", description = "Recupera la información de una persona específica utilizando su identificador único (UUID).")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.read', 'client.write')") // Solo los usuarios con autoridad
                                                                                      // 'admin.full' pueden acceder a
                                                                                      // esta información
        @GetMapping("/v1/api/{id}")
        public Object getPersonById(
                        @Parameter(description = "Identificador único de la persona (UUID)", required = true) @PathVariable UUID id) {
         ...
         */
        @Operation(summary = "Crear nueva persona", description = "Crea una nueva persona en el sistema a partir de los datos proporcionados.")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.write')") // Solo los usuarios con autoridad 'admin.full'
                                                                       // pueden crear personas
        @PostMapping("/v1/api")
         ...
         * y los transforma a una entidad de dominio para ser persistida. Luego,
         * convierte
         ...
         * @return ResponseEntity con el ingeniero registrado en formato de respuesta
         *         (DTO)
         */
        @Operation(summary = "Registrar nuevo ingeniero", description = "Registra una nueva persona con rol de ingeniero en el sistema a partir de los datos proporcionados.")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.write')") // Solo los usuarios con autoridad 'admin.full'
                                                                       // pueden registrar ingenieros
        @PostMapping("/vi/api/register/engineer")
        public ResponseEntity<?> registerEngineer(
                        @Valid @RequestBody PersonCreateRequest request) {
                return ResponseEntity.status(HttpStatus.CREATED)
                                .body(personRestMapper.toPersonResponse(
                                                personServicePort.registerEngineer(personRestMapper
                                                                .toPersonCreateRequestUseCase(request))));
        }

        /**
         * Registra un nuevo administrador en el sistema.
         *
         * Este endpoint recibe los datos de la persona con rol de administrador, los
         * valida
         * y los transforma a una entidad de dominio para ser persistida. Luego,
         * convierte
         * el resultado a un objeto de respuesta.
         ...
         * @return ResponseEntity con el administrador registrado en formato de
         *         respuesta (DTO)
         */
        @Operation(summary = "Registrar nuevo administrador", description = "Registra una nueva persona con rol de administrador en el sistema a partir de los datos proporcionados.")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.write')") // Solo los usuarios con autoridad 'admin.full'
                                                                       // pueden registrar administradores
        @PostMapping("/v1/api/register/admin")
        public ResponseEntity<?> registerAdimn(
                        @Valid @RequestBody PersonCreateRequest request) {
                return ResponseEntity.status(HttpStatus.CREATED)
                                .body(personRestMapper.toPersonResponse(
                                                personServicePort.registerAdmin(personRestMapper
                                                                .toPersonCreateRequestUseCase(request))));
        }
         ...
         *
         * Este endpoint recibe los datos de la persona con rol de CEO-Client, los
         * valida
         * y los transforma a una entidad de dominio para ser persistida. Luego,
         * convierte
         * el resultado a un objeto de respuesta.
         ...
         * @return ResponseEntity con el CEO-Client registrado en formato de respuesta
         *         (DTO)
         */
        @Operation(summary = "Registrar nuevo CEO-Client", description = "Registra una nueva persona con rol de CEO-Client en el sistema a partir de los datos proporcionados.")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.write')") // Solo los usuarios con autoridad 'admin.full'
                                                                       // pueden registrar CEO-Client
        @PostMapping("/v2/api/register/ceo-client")
        public ResponseEntity<?> registerCEOClient(
                        @Valid @RequestBody PersonCreateRequest request) {
                return ResponseEntity.status(HttpStatus.CREATED)
                                .body(personRestMapper.toPersonResponse(
                                                personServicePort.registerCEOClient(personRestMapper
                                                                .toPersonCreateRequestUseCase(request))));
        }
         ...
         * los valida y los transforma a un modelo de dominio para realizar la
         * actualización.
         ...
         * @return ResponseEntity con la persona actualizada en formato de respuesta
         *         (DTO)
         */
        @Operation(summary = "Actualizar persona", description = "Actualiza la información de una persona existente utilizando su identificador único (UUID) y los nuevos datos proporcionados.")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.write')") // Solo los usuarios con autoridad 'admin.full'
                                                                       // pueden actualizar personas
        @PutMapping("/v1/api/{id}")
        public ResponseEntity<PersonResponse> updatePerson(
                        @Parameter(description = "Identificador único de la persona a actualizar (UUID)", required = true) @PathVariable UUID id,
                        @Valid @RequestBody PersonUpdateRequest request) {
                return ResponseEntity.ok(
                                personRestMapper.toPersonResponse(
                                                personServicePort.update(id,
                                                                personRestMapper.tuUpdatePerson(request))));
        }
         ...
         * la operación de eliminación (Cambia el atributo b_estado_activo a false) en
         ...
         */
        @Operation(summary = "Eliminar persona", description = "Elimina una persona del sistema utilizando su identificador único (UUID).")
        @PreAuthorize("hasAnyAuthority('admin.full', 'client.write')") // Solo los usuarios con autoridad 'admin.full'
                                                                       // pueden eliminar personas
        @DeleteMapping("/v1/api/{id}")
        public ResponseEntity<Void> deletePerson(
                        @Parameter(description = "Identificador único de la persona a eliminar (UUID)", required = true) @PathVariable UUID id) {
